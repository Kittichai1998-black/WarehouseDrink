...
export default function AddStock() {
  ...
  const updateProduct = async () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .put("/api/"+mainPage+"/checkstock", {
            ID: Item.ID,
            UnitsOnOrder: countItem,
            UpdateBy: updateBy
          })
          .then((res) => {
  ...
  const calInventories = (data) => {
    return data.UnitsInStock - data.UnitsOnOrder
  }
      ...
      <div className="layout-page">
        <div className="card">
          ...
          {/* <div style={{ paddingTop: "16px" }}> */}
            <Card>
            <p class="w-2 text-left font-bold text-black-alpha-60 mr-3 text-3xl w-10">CheckStock</p>
          ...
          >
            <div className="p-inputgroup">
              <span className="p-inputgroup-addon" style={{width:"20rem"}}>Use</span>
              <InputNumber inputId="minmax" value={countItem} onValueChange={(e) => setCountItem(e.value)} />
            </div>
          </Dialog>
        </div>
      </div>
